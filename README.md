...
**13 September update:** The canonical invite-only preview at [maintainflow.io](https://maintainflow.io) was verified on `e446f6b`. Google consent, approved-value delivery to a private Sheet, replay without a duplicate row, and refresh after natural token expiry passed. Resend key/webhook setup and an own-inbox probe passed; its application environment is not applied and actual Folio intake remains unverified, with two exact approval requests pending. API-key expiry is implemented locally on `codex/api-key-expiry`: **238 serial tests**, typecheck and build passed, followed by **14 local browser checks**. The eight focused expiry tests and hosted packaging/runtime checks also passed after the browser fixes. It is not deployed and requires migration 020 before runtime promotion. Free hosting plans and mocked billing remain unchanged. See [current hosted status](docs/HOSTED-PREVIEW-STATUS-2026-09-13.md) and [API-key expiry behavior and release requirements](docs/API-KEY-EXPIRY.md).
...
| [HOSTED-PREVIEW-STATUS-2026-09-13.md](docs/HOSTED-PREVIEW-STATUS-2026-09-13.md) | Current hosted providers, local expiry work and remaining release gates |
