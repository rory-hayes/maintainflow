...
test('HTTP storage failures keep generic user errors and log only trusted diagnostic fields',async()=>{
 const client=createSupabaseStorage({url:'https://storage-fixture.supabase.co',serviceRoleKey:'PRIVATE synthetic credential',fetch:async()=>new Response(JSON.stringify({message:'PRIVATE vendor body',url:'https://private.example/?token=PRIVATE'}),{status:401})});
 let failure:unknown;
 ...
 try{
  ...
  assert.deepEqual(logs[0],{errorName:'Error',route:'/__test_storage_error',storageCode:'STORAGE_UPSTREAM_HTTP',storageOperation:'bucket-read',upstreamStatus:401});
