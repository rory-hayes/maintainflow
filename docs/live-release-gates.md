...
Apply migrations `001` through `011` in filename order before enabling the full
...
readiness-audit history and comparison evidence. The app verifies
...
`npm run test:db` now applies all eleven migrations to a uniquely named,
...
`CRON_SECRET`, verifies both storage layers, processes the oldest due accounts
in bounded batches, and returns aggregate counts without account identifiers.
...
The same protected daily invocation prunes readiness rate-limit buckets older
than 48 hours when that store is configured. Cleanup failure is logged but does
not discard completed monitoring outcomes.
