...
const source = JSON.parse(await fs.readFile('docs/evidence/openai-final-source-before.json', 'utf8'));
...
const freeze = {
  ...
  manifestSha256: digest(manifestBytes), runnerSha256: digest(await fs.readFile('docs/evidence/run-openai-heldout.ts')),
  fixtures: manifest.fixtures.map((f: any) => ({ id: f.id, filename: f.filename, sha256: f.sourceSha256 })),
  policy: 'Fixtures and expected values authored independently after the extractor was frozen, without reading extraction implementation, existing fixtures or reports. No code or prompt changes between cases. First-pass results retained, including failures. Small synthetic acceptance set, not customer accuracy evidence.',
};
await fs.writeFile(`docs/evidence/openai-heldout-freeze-${stamp}.json`, JSON.stringify(freeze, null, 2) + '\n', { flag: 'wx' });
...
const destination = `docs/evidence/openai-heldout-evaluation-${stamp}.json`;
