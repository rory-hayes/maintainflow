...
1. The private, empty `.local/hosted-preview/supabase-server-key.txt` is mode 0600 and ignored by Git/Vercel. The user must copy a current server Secret key from the existing Supabase project API Keys page into that file because Computer remains unavailable. Do not request the key in chat. Validate the key against the existing private bucket, then replace only the production `SUPABASE_SERVICE_ROLE_KEY` through the authenticated Vercel environment CLI and deploy a new protected candidate. No key replacement has been made yet.
