...
The app records **provider acceptance**, not inbox delivery. Automated report tests mock mail. Approved server configuration, hosted migration 026/current-recipient checks and production cron registration are complete. Actual confirmation and recovery emails reached the verified disposable owner's inbox with SPF/DKIM/DMARC pass; those Auth messages do not prove the separate report API or report delivery.

**Acceptance in progress, 7 September 2026:** health-only opt-in, verified-recipient binding and installation-condition preflight passed for the verified disposable owner. No report was sent: automatic approval review rejected sending pending explicit approval of the payload and recipient. Provider acceptance, inbox receipt and received-link opt-out remain pending. UI opt-out was saved and read-only database verification confirmed zero subscriptions or pending reports; both preferences are off. Owner-scoped manual maintenance and retention have passed with zero report messages accepted in that retention run. Actual scheduled execution and the first weekly delivery after seven days remain unproved; manual maintenance advances queue eligibility and is not cron evidence. See [the release ledger](maintaincode-release-status.md) for the confirmed result when available.
