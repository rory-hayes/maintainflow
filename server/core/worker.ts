...
let result:ExtractionResult;if(job.config.mode==='ai'){const activeProvider=provider;if(!activeProvider?.configured())throw Object.assign(new Error('AI extraction is not configured. Configure the server provider or choose text-anchor rules.'),{permanent:true});const bytes=await readStoredObject(data.doc.storage_key);result=await extractWithDeadline(activeProvider,{bytes,mimeType:data.doc.mime_type,pages:data.doc.source_text,schema:data.schema,instructions:job.config.instructions,locale:job.config.locale},options);}else{if(!data.doc.source_text.some((p:any)=>p.text.trim()))throw Object.assign(new Error('This document has no readable text. Scans and images require a configured OCR/AI provider.'),{permanent:true});result=extractRules(data.doc.source_text,data.schema,job.config.locale,job.config.templates||[]);}
