...
const {rows}=await c.query("select j.* from jobs j join workspaces w on w.id=j.workspace_id where j.state='queued' and j.available_at<=now() and ($1::uuid is not null or not exists(select 1 from schema_suggestions earlier where earlier.workspace_id=j.workspace_id and earlier.state='queued' and earlier.available_at<=now() and earlier.created_at<j.created_at)) and ($1::uuid is null or j.id=$1) and ((select count(*) from jobs running where running.workspace_id=j.workspace_id and running.state='processing')+(select count(*) from schema_suggestions s where s.workspace_id=j.workspace_id and s.state='processing')) < coalesce((w.plan->>'maxConcurrent')::int,2) order by j.created_at for update of j,w skip locked limit 1",[onlyJobId||null]);if(!rows[0])return null;const selected=rows[0];if(!await hasWorkspaceExtractionCapacity(c,selected.workspace_id))return null;await c.query("update jobs set state='processing',attempts=attempts+1,lease_owner=$2,lease_until=now()+interval '120 seconds',updated_at=now() where id=$1",[selected.id,owner]);await c.query("update documents set status='processing',error=null,updated_at=now() where id=$1",[selected.document_id]);return {...selected,attempts:selected.attempts+1};});
if(!job)return false;
try{
...
export async function enforceRetention(onlyWorkspaceId?:string,options:{signal?:AbortSignal;limit?:number}={}){
 ...
 for(const candidate of rows){
  ...
  const document=await withWorkspace(candidate.workspace_id,async c=>{
   // Recheck after the document lock: active extraction or field suggestions must not be purged.
