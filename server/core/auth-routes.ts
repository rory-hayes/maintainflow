...
app.post('/api/auth/password',{config:{rateLimit:authenticationRateLimit}},async req=>{const a=await requireActor(req);requireSession(a);const body=z.object({currentPassword:z.string().max(128),newPassword:z.string().min(10).max(128)}).parse(req.body);const user=(await adminPool.query('select password_hash from users where id=$1',[a.userId])).rows[0];if(!await verifyPassword(body.currentPassword,user.password_hash))badRequest('Current password is incorrect',400);const hash=await hashPassword(body.newPassword);await transaction(adminPool,async c=>{await c.query('update users set password_hash=$1 where id=$2',[hash,a.userId]);await c.query('delete from sessions where user_id=$1 and token_hash<>$2',[a.userId,hashToken(req.cookies.folio_session!)]);});return {ok:true};});
}
