...
// Share one bounded in-memory budget for login and registration in this process.
// Encapsulation keeps session reads and workspace routes outside this limiter.
await app.register(async authentication=>{
 await authentication.register(rateLimit,{
  global:true,hook:'onRequest',max:30,timeWindow:15*60_000,
  skipOnError:false,continueExceeding:false,exponentialBackoff:false,ban:-1,
  errorResponseBuilder:(_request,context)=>Object.assign(new Error('Too many authentication attempts. Try again in 15 minutes.'),{statusCode:context.statusCode}),
 });
authentication.post('/api/auth/register',async(req,reply)=>{authOrigin(req.headers.origin);const body=credentials.extend({name:z.string().trim().min(1).max(100),workspaceName:z.string().trim().min(1).max(100),inviteCode:z.string().max(256).optional()}).parse(req.body);if(!acceptsPreviewInvite(body.inviteCode))badRequest('An invite code is required for this test preview.',403);const passwordHash=await hashPassword(body.password);const ids=await transaction(adminPool,async c=>{if((await c.query('select 1 from users where email=$1',[body.email])).rowCount)badRequest('This email is already registered',409);const {rows:[u]}=await c.query('insert into users(email,name,password_hash) values($1,$2,$3) returning id',[body.email,body.name,passwordHash]);const {rows:[w]}=await c.query('insert into workspaces(name,slug,plan) values($1,$2,$3) returning id',[body.workspaceName,`${body.workspaceName.toLowerCase().replace(/[^a-z0-9]+/g,'-')}-${randomUUID().slice(0,8)}`,JSON.stringify(defaultPlan)]);await c.query('insert into memberships(workspace_id,user_id,role) values($1,$2,$3)',[w.id,u.id,'owner']);return {userId:u.id,workspaceId:w.id};});await createSession(reply,ids.userId,ids.workspaceId);reply.code(201);return sessionResponse(ids.userId,ids.workspaceId);});
authentication.post('/api/auth/login',async(req,reply)=>{
 ...
 await createSession(reply,user.id,member.workspace_id);
...
app.post('/api/auth/password',async req=>{const a=await requireActor(req);requireSession(a);const body=z.object({currentPassword:z.string().max(128),newPassword:z.string().min(10).max(128)}).parse(req.body);const user=(await adminPool.query('select password_hash from users where id=$1',[a.userId])).rows[0];if(!await verifyPassword(body.currentPassword,user.password_hash))badRequest('Current password is incorrect',400);const hash=await hashPassword(body.newPassword);await transaction(adminPool,async c=>{await c.query('update users set password_hash=$1 where id=$2',[hash,a.userId]);await c.query('delete from sessions where user_id=$1 and token_hash<>$2',[a.userId,hashToken(req.cookies.folio_session!)]);});return {ok:true};});
}
