...
function limit(ip:string){const now=Date.now();for(const [key,v]of attempts)if(v.until<now)attempts.delete(key);const item=attempts.get(ip)||{count:0,until:now+15*60000};item.count++;attempts.set(ip,item);if(item.count>30)badRequest('Too many authentication attempts. Try again in 15 minutes.',429);}
