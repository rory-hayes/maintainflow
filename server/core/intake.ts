...
import {inspectedFormat,ParserFormatNotAllowedError} from './intake-policy.js';
...
const source=await inspectSource(buffer,name);const format=inspectedFormat(source.mimeType);const sha=createHash('sha256').update(buffer).digest('hex');const id=randomUUID(),storageKey=`${actor.workspaceId}/${id}`;
...
let retained=false,writeAttempted=false,writeCompleted=false;try{writeAttempted=true;await privateStorage().write(storageKey,buffer);writeCompleted=true;const result=await withWorkspace(actor.workspaceId,async c=>{
...
if(idempotencyKey){const prior=await c.query('select d.*,e.id event_id,e.document_id prior_document_id,e.rejection_code,e.rejection_format,e.rejection_sha256,e.rejected_parser_id from intake_events e left join documents d on d.id=e.document_id where e.workspace_id=$1 and e.idempotency_key=$2',[actor.workspaceId,idempotencyKey]);if(prior.rows[0]){if(prior.rows[0].rejection_code){if(prior.rows[0].rejection_sha256!==sha||prior.rows[0].rejected_parser_id!==parserId)badRequest('This idempotency key was already used for a different document or parser.',409);return {rejection:new ParserFormatNotAllowedError(parserId,prior.rows[0].rejection_format)};}if(!prior.rows[0].prior_document_id)badRequest('This intake event was already handled and its document was deleted.',410);if(prior.rows[0].sha256!==sha||prior.rows[0].parser_id!==parserId)badRequest('This idempotency key was already used for a different document or parser.',409);return {document:camel(prior.rows[0]),duplicate:true,jobId:null};}}
...
await c.query('insert into usage_ledger(workspace_id,document_id,event,pages,idempotency_key) values($1,$2,$3,$4,$5)',[actor.workspaceId,id,'upload',source.pageCount,`upload:${id}`]);if(idempotencyKey)await c.query('insert into intake_events(workspace_id,idempotency_key,document_id) values($1,$2,$3)',[actor.workspaceId,idempotencyKey,id]);await audit(c,actor.workspaceId,actor.userId,'document.uploaded',id,{parserId,pages:source.pageCount});return {document:camel(doc),duplicate:false,jobId:job.id};});if('rejection' in result)throw result.rejection;retained=!result.duplicate;return result;}finally{clearInterval(heartbeat);await withWorkspace(actor.workspaceId,async c=>{if(!retained&&writeAttempted)await c.query("insert into file_deletions(workspace_id,storage_key,available_at) values($1,$2,now()+($3::int*interval '1 second')) on conflict(storage_key) do nothing",[actor.workspaceId,storageKey,!writeCompleted&&privateStorage().kind==='supabase'?300:0]);await c.query('delete from intake_files where id=$1',[id]);});if(!retained&&writeAttempted&&(writeCompleted||privateStorage().kind==='filesystem'))await deleteStoredFile(actor.workspaceId,storageKey);}
}
