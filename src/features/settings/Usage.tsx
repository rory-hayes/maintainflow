...
type UsageData = {
  ...
  usage: { pages: number; events: number; documents: number; needsReview: number; processed: number; failed: number; costUsd: number; extractionCostUsd?: number; schemaSuggestionCostUsd?: number; schemaSuggestions?: number };
  ...
  return (
    <div className="workspace-usage">
      ...
      {tab === 'Billing' ? <BillingPanel /> : <>
        ...
        <p className="small muted settings-bottom-note">Estimates cover recorded successful extractions and field suggestions, separate from your page allowance. Charges for failed or canceled attempts may not be included. This is not a reconciliation of the provider’s bill.</p>
        <p className="small muted">Field suggestions: {usage.schemaSuggestions ?? 0} completed this month · {new Intl.NumberFormat('en-US', {style:'currency', currency:'USD', minimumFractionDigits:4}).format(usage.schemaSuggestionCostUsd ?? 0)} estimated model cost. Suggestions use no additional page credits.</p>
