...
type Parser={id:string;name:string;archived:boolean;mode:'rules'|'ai';allowedFormats?:SourceFormat[]|null;fieldSetupState?:string};
...
export default function Upload({parserId,onComplete,compact=false}:{parserId?:string;onComplete?:()=>void;compact?:boolean}){
  ...
  const splitParser=parserId?query.data?.parsers.find(parser=>parser.id===parserId):chosenParser;
  const acceptedFormats=chosenParser?.allowedFormats;
  const formatsHint=acceptedFormats?`Accepted formats: ${sourceFormats.filter(format=>acceptedFormats.includes(format.id)).map(format=>format.label).join(', ')}`:'All supported formats';
